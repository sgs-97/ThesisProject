...
function show_help() {
    echo "Description:"
    echo "  Run hmd_umount_log_to_sleep_log_durations.py on the specified directory containing log and annotation (laps) data after being preprocessed."
    echo
...
function main() {
    ...
    echo "Generating HMD umount log to sleep log durations for directory: $dir"
    ...
    python3 "$SCRIPT_DIR"/../analyze/hmd_umount_log_to_sleep_log_durations.py "$dir"
