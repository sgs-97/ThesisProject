...
function show_help() {
    echo "Description:"
    echo "  Run hmd_umount_lap_to_sleep_lap_durations.py on the specified directory containing log and annotation (laps) data after being preprocessed."
    echo
...
function main() {
    ...
    echo "Generating HMD umount lap to sleep lap durations for directory: $dir"
    ...
    python3 "$SCRIPT_DIR"/../analyze/hmd_umount_lap_to_sleep_lap_durations.py "$dir"
