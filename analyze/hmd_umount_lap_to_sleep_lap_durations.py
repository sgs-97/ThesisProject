...
if __name__ == '__main__':
    ...
    umount_lap_to_sleep_lap_durations = abs(pd.Timestamp(hmd_sleep_time) - pd.Timestamp(hmd_umount_time)).total_seconds() if (hmd_umount_time and hmd_sleep_time) else None

    # Report oddly large durations
    if umount_lap_to_sleep_lap_durations is not None and umount_lap_to_sleep_lap_durations > 10:
        print(f"[\033[1;31mWARNING\033[0m] Unmount to Passthrough Start Duration for: {os.path.dirname(exp_dir)}/{os.path.basename(exp_dir)} is unusually large: {umount_lap_to_sleep_lap_durations} seconds.")
