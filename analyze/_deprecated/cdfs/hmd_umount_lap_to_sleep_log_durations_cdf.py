...
if __name__ == "__main__":
    ...
    parser.add_argument("--variable_for_cdf", default=["umount_lap_to_sleep_log_duration"], help="The variable to use for the CDF. (Default: umount_lap_to_sleep_log_durations)")
    ...
    for idx, csv_file in enumerate(args.csv_files):
        ...
        values = df["umount_lap_to_sleep_log_duration"].astype(float)
        label = args.legend_labels[idx] if args.legend_labels and idx < len(args.legend_labels) else f"{os.path.splitext(os.path.basename(csv_file))[0].replace('_',' ')} – umount_lap_to_sleep_log_duration"
    ...
    stats_extension = 'txt'
    base_name = "hmd_umount_lap_to_sleep_log_durations_cdf"
