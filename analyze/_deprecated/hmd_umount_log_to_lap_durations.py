...
def get_hmd_umount_log_lap_times(events_json):
    """
    Extracts HMD unmount (log) and (lap) times from the events JSON data.
    """
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Extracts the durations between system-logged HMD unmount (log) and externally lapped (lap) times from an experiment directory.')
    ...
    umount_log_to_lap_duration = abs(pd.Timestamp(hmd_umount_lap_time) - pd.Timestamp(hmd_umount_log_time)).total_seconds() if (hmd_umount_log_time and hmd_umount_lap_time) else None

    # Report oddly large durations
    if umount_log_to_lap_duration is not None and umount_log_to_lap_duration > 10:
        print(f"[\033[1;31mWARNING\033[0m] Unmount to Passthrough Start Duration for: {os.path.dirname(exp_dir)}/{os.path.basename(exp_dir)} is unusually large: {umount_log_to_lap_duration} seconds.")
    
    # Write to file (hmd_umount_log_to_lap_durations.csv)
