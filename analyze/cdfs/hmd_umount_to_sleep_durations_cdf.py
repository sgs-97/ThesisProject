...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate CDF graph(s) from one or more HMD unmount to sleep duration CSV files into a matplotlib/plotly graph (output is png/HTML).")
    parser.add_argument("csv_files", nargs='+', help="Path(s) to the input combined CSV(s).")
    parser.add_argument("--variable_for_cdf", default=["umount_to_sleep_duration"], help="The variable to use for the CDF. (Default: umount_to_sleep_duration)")
    ...
    for idx, csv_file in enumerate(args.csv_files):
        ...
        values = df["umount_to_sleep_duration"].astype(float)
        label = args.legend_labels[idx] if args.legend_labels and idx < len(args.legend_labels) else f"{os.path.splitext(os.path.basename(csv_file))[0].replace('_',' ')} – umount_to_sleep_duration"
    ...
    stats_extension = 'txt'
    base_name = "hmd_umount_to_sleep_duration_cdf"
    ...
    if args.graphing_tool == "matplotlib":
        matplotlib_cdf_multi(
            ...
            title="CDF of HMD Unmount to Sleep Durations",
