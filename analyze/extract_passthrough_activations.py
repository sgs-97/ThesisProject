...
if __name__ == '__main__':
    ...
    # Get start rec time
    events_json_fpath = os.path.join(experiment_dir, f'{os.path.basename(experiment_dir)}.json')
    ...
    with open(output_csv_fpath, 'w') as f:
        f.write("sensor_name,type,time,rel_time_to_start_rec\n")
        ...
        for sensor_name, _ in sensor_events.items():
            if sensor_name in ['passthrough', 'Passthrough']:
                ...
                for sensor_event in sensors_events_list:
                    ...
                    f.write(f"{sensor_name},{sensor_event['Type']},{sensor_event['Time']},{rel_time_to_start_rec}\n")
