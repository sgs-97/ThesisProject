...
if __name__ == "__main__":
    ...
    if variable_for_cdf == "unmount_imx471_start" or variable_for_cdf == "all": # there is a duration column so we directly utilize it to calculate the cdf
        output_file = os.path.dirname(csv_file) + f"/umount_imx471_start_duration_cdf.{extension}"
        ...
        if graphing_tool == "matplotlib":
            matplotlib_cdf(durations, output_file, export_stats=export_stats, stats_txt_path=output_stats_file, title="CDF of HMD Unmount to imx471 Start Durations")
        else:
            plotly_cdf(durations, output_file, export_stats=export_stats)

    if variable_for_cdf == "imx471_stop_sleep" or variable_for_cdf == "all": # The frequency has to be calculated asfrom the "starts". However since this is a concatenated csv we have to check for the source_file_column to be the same each time we add a value to the total
        output_file = os.path.dirname(csv_file) + f"/imx471_stop_sleep_duration_cdf.{extension}"
        ...
        if graphing_tool == "matplotlib":
            matplotlib_cdf(durations, output_file, export_stats=export_stats, stats_txt_path=output_stats_file, title="CDF of imx471 Stop to HMD Sleep Durations")
        else:
            plotly_cdf(durations, output_file, export_stats=export_stats)